...
## Features
- Supports plain C99, and C++11 with extensive operator overloading, type traits, templated conversions etc.
- Full test coverage.
- Type-safe C++ bit-masking / comparison.
- The C++ classes are written in terms of the C implementation.
- The C++ classes are (mostly) immutable, apart from the `+=`, `*=` operators etc. Most non-static methods are marked `const`.
...
- The generic, non-vector implementation allows for compilation on any target. (This implementation can be forced by defining `SIMD_GRANODI_FORCE_GENERIC`). It also serves as documentation for those not familiar with SIMD intrinsics.
...
This library was written so that the author could easily write cross-platform Audio DSP code that runs on all x86_64 machines, and newer AArch64 machines (ie latest hardware from Apple). It targets 128-bit SIMD types (32x4 or 64x2).

The x86_64 implementation limits itself to SSE2, as many otherwise-capable modern low-end CPUs do not support AVX, SSE2 is guaranteed to be implemented on all x86_64 machines, and later SSE instructions (eg SSE 4.2) only provide a marginal speed improvement to the functionality of this library. Also, NEON only supports 128-bit vectors. However, it would be very easy to add AVX support in future, and simply wrap 2 or 4 128-bit registers on NEON.

### Slow / non-vector functions

Some platforms do not have intrinsic functions for some SIMD operations. A list of these functions/macros, per-platform, is in a comment at the start of the `simd_granodi.h` file. If you are using the C++ classes, you may wish to search for those names in the file to see which methods they correspond to.
...
- All vector types are default-constructed to hold a value of zero. This is for safety and convenience, and typically gets optimized out.
...
- All vector types have a "broadcast" constructor that takes a single value and "broadcasts" it to all elements of the vector. For example, `Vec_ps{3.0f}` will result in the vector `{3.0f, 3.0f, 3.0f, 3.0f}`. This also allows for convenient arithmetic with constants, for example `Vec_ps{3.0f} + 1.0f` will give the vector `{4.0f, 4.0f, 4.0f, 4.0f}` as the `1.0f` is implicitly constructed into a `Vec_ps`.
...
- All comparison types also have a "broadcast" constructor which accepts a `bool`. For vector comparisons, all the bits of the bit-mask are set to `0` if this is `false`, or `1` if this is true. For the "scalar wrapper" types, this sets the value of the `bool` member.
...
The elements of a vector can be accessed with the templated `get<int32_t>()` method. The index is passed as a template argument. An out of range index will cause a compile time error. Example: `Vec_pd{4.0, 3.0}.get<1>()` will return a `double` with value `4.0`.
...
All vector types implement the following standard arithmetic operators: `+=`, `+`, `-=`, `-`, `*=`, `*`, `/=`, `/`. In addition, integer types support both the pre- and post-fix `++` and `--` operators.
...
When a comparison operator is used, it returns a result of the corresponding `Compare_` type. All vector types implement the following comparison operators: `<`, `<=`, `==`, `!=`, `>=`, `>`.

### Logical operators for `Compare_` types

All of the comparison types support the following logical operators: `&&`, `||`, `!`.
...
For the 128-bit vector types, the `.choose()` and `.choose_else_zero()` methods simply use bit-masking. The advantage is that this is completely branch-less, but the disadvantage is that both "branches" or possibilities are calculated: the unneeded result is discarded. This may be expensive and these methods should be used carefully.
...
All signed integer types implement bit-shifting methods. These are not implemented for floating point types, but can be used via bitcasting if needed. For shifting by immediate values of type `int32_t` (shifting by a compile-time constant) these are:
...
- `.shift_l(const Vec_& amount)` - Return a new vector with each element shifted left by `amount`.
- `.shift_rl(const Vec_& amount)` - Return a new vector with each element shifted right logically by `amount`.
- `.shift_ra(const Vec_& amount)` - Return a new vector with each element shifted right arithmetically by `amount`.
...
Any `Vec_` type can be bitcasted to any other `Vec_` type of the same total size. (The elements do not need to be the same size, but the total size of the vector must be). This is possible using the `.bitcast<typename To>()` method. Eg `Vec_ps{4.0f}.bitcast<Vec_pi64>()` will re-interpret 4 packed 32-bit floating point values as 2 packed 64-bit signed integers. This bitcast is allowed because they are both 128-bit types.

You can **not** bitcast a `Compare_` type to any other type, but you can convert between `Compare_` types.
